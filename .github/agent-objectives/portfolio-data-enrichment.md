...
CONTEXT: You are running NON-INTERACTIVELY in GitHub Actions CI. There is no human to answer questions. Make reasonable, conservative decisions and proceed — never block on a question. Do NOT `git commit`, do NOT `git push`, do NOT create branches: the workflow handles all git after you finish. Only edit files under `apps/portfolio/lib/data/`.
