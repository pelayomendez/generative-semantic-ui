...
- When asked about a SPECIFIC project (e.g. "tell me about Mugaritz", "what was Parsifal?"), render a rich detail view inside a \`<Section title="<Project name> (<year> · <location>)">\` wrapping a \`<Stack gap={8}>\` with, IN THIS ORDER: (1) a HERO MEDIA block — that project's OWN \`<Video>\` using its exact \`projects[i].video\` URL verbatim (never another project's); if it has no \`video\`, use its own \`images[0]\` as an \`<Image>\` instead; if it has neither, omit the hero. (2) A PROSE↔META split: a \`<Grid cols={3} gap={8}>\` whose FIRST cell is a \`<Stack gap={4} span={2}>\` (the \`span={2}\` makes the prose ~2× the meta width — an 8/4 split) containing a \`<Paragraph>\` of the summary inlined verbatim, and whose SECOND cell is a \`<Stack gap={6}>\` of metadata blocks — each block a \`<Stack gap={2}>\` with a \`<Heading level={4}>\` label and its values: a "Role" block (\`<Paragraph>\` of the \`role\`), a "Collaborators" block (a \`<Row gap={2}>\` of \`<Badge variant="outline">\` — one per name in the project's \`collaborators\` array; OMIT this block entirely if the project has no \`collaborators\`), and a "Tags" block (a \`<Row gap={2}>\` of \`<Badge>\` from \`tags\`). (3) SUPPORTING MEDIA: if the project has more than one image, a \`<Grid cols={2} gap={6}>\` of \`<Image>\` elements for the project's OWN \`images[1..]\` (paste each URL verbatim; omit this grid if there are no further images). Use ONLY that one project's own media and facts — never borrow from another entry.
