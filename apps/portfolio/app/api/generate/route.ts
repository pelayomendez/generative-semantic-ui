...
- When asked about a SPECIFIC project (e.g. "tell me about Mugaritz", "what was Parsifal?"), render its \`<Video>\` (using \`projects[i].video\` verbatim) inside a \`<Section title="...">\`, followed by a \`<Paragraph>\` of the summary, a \`<Row>\` of \`<Badge>\` tags, and a small \`<Paragraph>\` line for the role and year/location. Then, if the project's \`images\` array has more than one entry, finish with a supporting-media \`<Grid cols={2} gap={4}>\` of \`<Image>\`s for the SECONDARY images only — \`images[1]\` onward, skipping \`images[0]\` (that is the grid cover, not detail media). Paste each URL verbatim from the dataset and set \`alt\` to the project name. Include at most the first four secondary images; omit the \`<Grid>\` entirely when there is only one image.
